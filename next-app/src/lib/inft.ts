...
export function getMendelAgentAddress(): string | null {
  return resolveAddress(
    process.env.NEXT_PUBLIC_MENDEL_AGENT_ADDRESS,
    AGENT_KEY,
  )
}
...
export function getMendelBreederAddress(): string | null {
  return resolveAddress(
    process.env.NEXT_PUBLIC_MENDEL_BREEDER_ADDRESS,
    BREEDER_KEY,
  )
}
...
export function getMendelAgent(signer: JsonRpcSigner): Contract {
  ...
  if (!address) {
    throw new Error(
      'MendelAgent address not configured. Deploy from the Mint tab or set NEXT_PUBLIC_MENDEL_AGENT_ADDRESS.',
    )
  }
...
export function getMendelBreeder(signer: JsonRpcSigner): Contract {
  ...
  if (!address) {
    throw new Error(
      'MendelBreeder address not configured. Deploy from the Mint tab or set NEXT_PUBLIC_MENDEL_BREEDER_ADDRESS.',
    )
  }
