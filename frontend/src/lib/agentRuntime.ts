...
//
// In dev, both apps run side-by-side: Vite on 5173, Next on 3001.
// In prod, point VITE_AGENT_RUNTIME_URL at the deployed runtime URL.

import type { Genome } from './genome'

const BASE =
  (import.meta.env.VITE_AGENT_RUNTIME_URL as string | undefined)?.replace(
    /\/$/,
    '',
  ) ?? 'http://localhost:3001'
